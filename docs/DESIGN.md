...
`inventory.yaml` is read by `yaml.py`, a stand-in that parses the subset an inventory is made of — nested mappings, block sequences, scalars, comments — with the standard library alone. Vendoring PyYAML into the repo would also have worked, and was rejected: several thousand lines of someone else's code to review and keep current, for one function call, in a repo whose whole claim is that it can be read before it is trusted.

The file held JSON first, on the grounds that JSON is a subset of YAML 1.2 and therefore parses under either. That worked and was unpleasant to live in: the one file a site has to edit by hand was the one file that could not carry a `#` comment, so every explanation of what a key does had to be smuggled into a `_comments` object that the loader then had to ignore. The parser is the better place to spend the complexity — a hundred lines, against a schema this repo defines and a test suite that checks it agrees with real PyYAML on the shipped template. JSON is still accepted, because inventories written before this are JSON and there is no reason to break them.

The cost is that `yaml.py` shadows an installed PyYAML for anything run from `scripts/`, where it sits beside the modules that import it. That shadowing is now deliberate rather than tolerated: one parser everywhere means an inventory that loads on the locked-down work machine loads identically on a developer's box, which matters more for a compliance tool than reaching the fuller dialect PyYAML would offer. The subset is the price — anchors, block scalars and multiple documents are refused by line number rather than guessed at, since a quietly mis-read inventory does not look like an error, it looks like a verdict.
...
Where the config sources interface templates, the `show template interface source user <name>` reads are named on the **interface** rules and nowhere else. Every check is wrapped by `through_templates`, which is why this once named them on all sixty-odd — true about the code and false about the rules. Expanding a template inserts its body into an interface block and changes nothing else, so a rule grepping `^logging userinfo` reaches the identical verdict either way, and naming the template read under it claimed the template mattered to a verdict it could not have moved — in every comment box in the checklist. Which rules qualify is taken from `RULE_VERIFY`: a rule whose evidence is shown by `show running-config | section ^interface` is exactly a rule a template can change, so there is one list rather than two that can drift.
