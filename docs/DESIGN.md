...
## Dependencies, and what runs without them

netmiko and PyYAML are real dependencies here — `requirements.txt`, installed with pip. That is the difference between this repository and the offline-only STIG sibling, which carries a stdlib stand-in for PyYAML because pip is not an option on the host it runs from. Nothing below assumes that stand-in exists.

`inventory.yaml` still holds JSON rather than YAML. JSON is a subset of YAML 1.2, so PyYAML parses it unchanged, and writing it as JSON keeps it readable by anything with a `json` module if this ever has to run somewhere leaner.

Netmiko is imported inside `netauto.connect()` rather than at module scope, so nothing on the audit path loads it. `--from-capture` runs without ever opening a connection. The SecureCRT collectors go further and import nothing from this repository at all — they are copied onto the host as single files, and they need only Python.
