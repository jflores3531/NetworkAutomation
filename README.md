...
Development and the hardening scripts are validated against a 7-device virtual lab (2 IOS routers, 3 IOSvL2 switches, 2 NX-OS cores), rebuildable from [`lab/`](lab/). Ansible roles under [`ansible/`](ansible/) replicate the Python hardening for fleet-wide runs. See [`docs/DESIGN.md`](docs/DESIGN.md) for the reasoning behind script isolation, run order, and credential handling.
...
### General-purpose device operations
Not STIG work — the day-to-day tooling this repo keeps alongside it.
...
| `l2_quiet_console.py` | **Configures devices.** Disable live console/monitor logging. Quality-of-life, not a STIG item; messages still buffer and forward to syslog. |
| `l2_device_tracking.py` | **Configures devices.** SISF `device-tracking policy` for host IP visibility. Not a STIG requirement, IOS-XE only. |
...
# Optional, non-STIG
python3 scripts/l2_quiet_console.py S1      # quiet the console during interactive config work
python3 scripts/l2_device_tracking.py S1    # IOS-XE only, host IP visibility

# Rebuild the lab from lab/topology.yaml, or just check it against the file
python3 lab/rebuild_lab.py
python3 lab/rebuild_lab.py --verify
